<script lang="ts">
	...
	let { linkedin = "", github = "", facebook = "" }: Props = $props();
	...
	onMount(() => {
		...
		const element = document.querySelector('.social-section');
	...
	const socials = [
		{ name: 'LinkedIn', url: linkedin, icon: 'linkedin', color: '#0077b5' },
		{ name: 'GitHub', url: github, icon: 'github', color: '#333' },
		{ name: 'Facebook', url: facebook, icon: 'facebook', color: '#1877f2' }
	].filter(s => s.url);
</script>
...
<section class="social-section section">
	<div class="container">
		{#if visible}
			<div class="social-links" in:fade={{ duration: 600 }}>
				{#each socials as social, i}
					<a
						...
						class="social-link"
						in:scale={{ duration: 400, delay: i * 100 + 200 }}
...
<style>
	...
	.social-link::before {
		...
		width: 100%;
		height: 100%;
		background: linear-gradient(90deg, transparent, rgba(245, 225, 89, 0.1), transparent);
	...
	.social-link:hover {
		...
		box-shadow: 0 10px 30px rgba(245, 225, 89, 0.2);
	}
